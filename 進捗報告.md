...
グリッドサーチを用いて CNN, NaiveBayes のハイパーパラメータの調整をする．
しかしコードは書きましたが，学習が授業中に終わりませんでした．

#### 次回の目標

学習が終わり次第精度向上に努めるのか，UX 向上に努めるのか判断します．
...
フィッシングサイト検知に関する[論文](https://arxiv.org/abs/1802.03162)を軽く読みました．

> URLNet: Learning a URL Representation with Deep Learning for Malicious URL Detection _Hung Le and Quang Pham and Doyen Sahoo and Steven C. H. Hoi_ arXiv 1802.03162 2018

簡単に要約すると以下のようになっていました．

```txt
URLを学習するエンドツーエンドの深層学習フレームワークであるURLNetを提案します．具体的には，畳み込みニューラルネットワークをURL文字列の文字と単語の両方に適用し，共同で最適化されたフレームワークでURLエンベッディングを学習します．
```

またこの論文で作成された[URLNet](https://github.com/Antimalweb/URLNet)を実行しようとしたのですが，環境が合わず実行できませんでした．

#### 次回の目標

本論文から Word-lebel CNN, Charctor-lebel CNN が有効であることがわかったので，CNN の実装を行います．
...
先週読んだ論文に書かれている Word-level CNN, Charctor-level CNN を混ぜたような CNN は授業時間内にうまく実装できそうにないので，本日は「学習結果の可視化」，「最新バージョンをPaaSへデプロイ」を行いました．

![学習結果](images/classification.png)
